...
extension CreateAdDraft {
    ...
    static func prefilled(from ann: AnnouncementDTO) -> CreateAdDraft {
        ...
        d.budgetMin = ann.budgetMinValue.map(String.init) ?? ""
        d.budgetMax = ann.budgetMaxValue.map(String.init) ?? ""
        if d.budgetMin.isEmpty, d.budgetMax.isEmpty {
            let legacyBudget = ann.data["budget"]?.stringValue ?? ann.data["budget"]?.intValue.map { String($0) } ?? ""
            d.budgetMin = legacyBudget
            d.budgetMax = legacyBudget
        }
