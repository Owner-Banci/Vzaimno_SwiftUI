...
final class CreateAdDraft: ObservableObject {
    ...
    func validateMainStepSync() -> String? {
        ...
        if let e = AdValidators.validateBudgetRange(min: budgetMin, max: budgetMax) { return e }
...
//        let titleTrimmed = AdValidators.trimmed(title)
        ...
        if let compatibleBudget = budgetCompatibilityValue() ?? AdValidators.parseDecimal(budget).map(Int.init) {
