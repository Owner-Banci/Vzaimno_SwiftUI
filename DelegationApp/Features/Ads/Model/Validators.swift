...
enum AdValidators {
    ...
    static func validateBudgetRange(min minValue: String, max maxValue: String) -> String? {
        if let error = validateBudget(minValue) { return "Минимальный бюджет: \(error.lowercased())" }
        if let error = validateBudget(maxValue) { return "Максимальный бюджет: \(error.lowercased())" }

        let minNumber = parseDecimal(minValue)
        let maxNumber = parseDecimal(maxValue)

        if let minNumber, let maxNumber, minNumber > maxNumber {
